      ...
      <main className="flex-1 px-4 md:px-6 py-4 md:py-6 flex flex-col gap-5 max-w-5xl w-full mx-auto">
        ...
        {term && mascotas.length > 0 && (
          <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {/* Encabezado de columnas: solo en escritorio, en el celular cada
                dato lleva su propia etiqueta dentro de la tarjeta. */}
            <div className="hidden md:grid grid-cols-[1.2fr_1.4fr_auto] gap-4 px-5 py-2 border-b border-slate-100 text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
              <span>Mascota</span>
              <span>Dueño</span>
              <span className="text-right">Última consulta</span>
            </div>
            <div className="divide-y divide-slate-100">
              {mascotas.map(m => (
                <div
                  ...
                  // En el celular: dos columnas fijas — la mascota y, al lado,
                  // su dueño. Con `flex-wrap` las columnas se desalineaban de
                  // fila en fila y no se entendía qué dueño era de quién.
                  className="grid grid-cols-2 md:grid-cols-[1.2fr_1.4fr_auto] gap-x-4 gap-y-2 px-4 md:px-5 py-3 hover:bg-emerald-50/40 transition cursor-pointer"
                >
                  {/* Mascota */}
                  ...
                  {/* Dueño */}
                  <div className="min-w-0">
                    <span className="md:hidden text-[10px] font-semibold text-slate-400 uppercase tracking-wide block">
                      Dueño
                    </span>
                    <button
                      onClick={e => { e.stopPropagation(); navigate(`/clientes/${m.cliente_id}`) }}
                      className="w-full text-sm text-purple-700 hover:text-purple-900 font-medium underline-offset-2 hover:underline text-left truncate block"
                    >
                      {m.propietario || '—'}
                    </button>
                    <p className="text-xs text-slate-500 truncate">
                  ...
                  {/* Última consulta + acceso a la historia. En el celular
                      ocupa el ancho completo debajo, como pie de la tarjeta. */}
                  <div className="col-span-2 md:col-span-1 flex items-center justify-between md:justify-end gap-3 md:flex-col md:items-end md:gap-1">
                    <span className="text-xs text-slate-500 whitespace-nowrap">
                      <span className="md:hidden text-slate-400">Última consulta: </span>
                      {m.ultima_consulta
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 hover:text-emerald-900 whitespace-nowrap"
                    >
                      <Stethoscope className="w-3.5 h-3.5 shrink-0" /> Ver historia
                    </button>
                  </div>
                </div>
              ))}
